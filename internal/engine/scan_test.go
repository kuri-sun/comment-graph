...
`)
	...
	if len(errs) != 0 {
		t.Fatalf("expected autogenerated id with no errors, got %+v", errs)
	}
	if len(g.Todos) != 1 {
	...
	if !strings.HasPrefix(id, "todo-a-go-1") {
		t.Fatalf("expected generated id prefix todo-a-go-1, got %s", id)
	}
}
